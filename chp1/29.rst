...
另外一个需要注意的地方是，辛普森函数在调用之前使用 `exact->inexact <http://www.gnu.org/software/mit-scheme/documentation/mit-scheme-ref/Numerical-operations.html#Numerical-operations>`_ 函数对参数 ``a`` 进行了转换，从而确保函数的计算结果也是浮点数格式的（更多信息参考书本 15 页，注释 23 ）。
