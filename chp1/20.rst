...
可以看到，对于应用序求值来说， ``(gcd 206 40)`` 共使用 4 次 ``remainder`` 调用。
...
每次代换的时候都需要对b求值，对应上面的过程就是t0、t1、t2、t3、t4，最后一次需要对a求值对应t3，故times = t0 + t1 + t2 + t3 + t4 + t3。

times = 0 + 1 + 2 + 4 + 7 + 4 = 18，故对于正则序求值来说， ``(gcd 206 40)`` 共使用 18 次 ``remainder`` 调用。
