...
其中函数 :math:`cf(i)` 表示连分式其余的 :math:`k-i+1` 个除法项计算所得之商。

根据这个递归表达式，我们可以给出（递归计算版本）连分式过程的定义：

.. literalinclude:: code/37-recursive-cont-frac.scm
...
.. literalinclude:: code/37-iterative-cont-frac.scm
...
::
    ...
    ;  Loading "37-recursive-cont-frac.scm"... done
    ;... done
    ;Value: golden-ratio
...
从测试结果可以看出，只要 :math:`k` 的值大于等于 :math:`11` ，就可以保证黄金分割率的前四位精度： :math:`1.618` 。
...
::
    ...
    1 ]=> (load "37-iterative-cont-frac.scm")

    ;Loading "37-iterative-cont-frac.scm"... done
    ;Value: cont-frac

    1 ]=> (define (golden-ratio k)
            (+ 1
