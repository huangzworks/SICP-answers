...
.. literalinclude:: code/22-square-list.scm

使用 ``(list 1 2 3 4)`` 作为输入，测试 ``square-list`` 函数：

::

    1 ]=> (load "22-square-list.scm")

    ;Loading "22-square-list.scm"... done
    ;Value: square-list
...
.. literalinclude:: code/22-another-square-list.scm

将 ``(list 1 2 3 4)`` 作为输入，对 ``square-list`` 进行测试：

::

    1 ]=> (load "22-another-square-list.scm")

    ;Loading "22-another-square-list.scm"... done
    ;Value: square-list
