...
.. literalinclude:: code/p74-make-mobile-and-make-branch.scm
...
::
    ...
    1 ]=> (define mobile (make-mobile (make-branch 10 25)       ; 创建活动体
                                      (make-branch 5 20)))

    ;Value: mobile

    1 ]=> (left-branch mobile)      ; 左分支

    ;Value 12: (10 25)

    1 ]=> (right-branch mobile)     ; 右分支

    ;Value 13: (5 20)

    1 ]=> (branch-length (right-branch mobile))     ; 右分支的长度

    ;Value: 5

    1 ]=> (branch-structure (right-branch mobile))  ; 右分支的重量
...
.. literalinclude:: code/29-mobile-blance.scm

测试：

::

    1 ]=> (define blance-mobile (make-mobile (make-branch 10 10)
                                             (make-branch 10 10)))

    ;Value: blance-mobile

    1 ]=> (mobile-blance? blance-mobile)

    ;Value: #t

    1 ]=> (define unblance-mobile (make-mobile (make-branch 0 0) 
                                               (make-branch 10 10)))

    ;Value: unblance-mobile

    1 ]=> (mobile-blance? unblance-mobile)

    ;Value: #f
    
    1 ]=> (define mobile-with-sub-mobile (make-mobile (make-branch 10 blance-mobile)
                                                      (make-branch 10 blance-mobile)))

    ;Value: mobile-with-sub-mobile

    1 ]=> (mobile-blance? mobile-with-sub-mobile)
...
我们的活动体程序通过实现数据抽象的方式，将程序之间的关系很好地用构造函数和选择函数隔离开了，就算 ``make-mobile`` 和 ``make-branch`` 这两个构造函数使用新的表示方式，我们只需修改相应的选择函数，就可以让 ``mobile-blance?`` 等程序继续运行在新表示之下。
...
.. literalinclude:: code/p75-new-constructor.scm

测试：

::

    1 ]=> (load "29-mobile-blance.scm")

    ;Loading "29-mobile-blance.scm"...
    ...
    1 ]=> (load "29-new-selector.scm")          ; 记得将新的构造函数和选择函数放到最后载入

    ;Loading "29-new-selector.scm"... done
    ;Value: branch-structure

    1 ]=> (load "p75-new-constructor.scm")

    ;Loading "p75-new-constructor.scm"... done
    ;Value: make-branch
    ...
    1 ]=> mobile                    ; 确认使用的是新的表示方式

    ;Value 11: ((10 . 20) 10 . 20)

    1 ]=> (mobile-blance? mobile)
