...
使用 ``filtered-accumulate`` ，我们可以定义一个 ``primes-sum`` ，它加起给定范围内的所有素数，其中素数检测函数 ``prime?`` 来自书本 33 页：
...
使用书本 32 页介绍的 ``gcd`` 函数(mit-scheme 也内置了这个函数)，以及题目给出的互素性质，我们可以写出函数 ``coprime?`` ，用于检测两个整数是否互素：
...
注意，因为 ``coprime?`` 函数需要两个参数，所以在 ``accumulate`` 函数的最后部分，使用了一个 ``lambda`` 将参数 ``n`` 闭包进去，作为 ``coprime?`` 的第二个参数。
...
.. literalinclude:: code/33-iterative-filtered-accumulate.scm

测试：

::

    1 ]=> (load "33-iterative-filtered-accumulate.scm")

    ;Loading "33-iterative-filtered-accumulate.scm"... done
