...
scheme 数值包程序可以直接在 :doc:`79` 的源码的基础上进行修改，一个值 ``value`` 对于 ``=zero?`` 为真当且仅当这个值对于 ``(equ? value 0)`` 为真：
...
有理数包程序可以在书本 129 页的 ``(install-rational-package)`` 源码的基础上进行修改，一个有理数为零当且仅当它的分母为 ``0`` ：
...
::
    ...
    1 ]=> (load "80-complex-driver.scm")

    ;Loading "80-complex-driver.scm"...
    ...
    ;  ... done
    ;  Loading "p125-apply-generic.scm"... done
    ;  Loading "p125-generic-selector.scm"... done
