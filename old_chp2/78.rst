...
::
    ...
    ;Value 12: (scheme-number . 10)


题目要求我们修改 ``attach-tag`` 、 ``type-tag`` 和 ``contents`` 函数，使得可以直接使用 scheme 的内置数字类型。

为了做到这一点，需要修改以上函数，让它们分别加上一条 scheme 数字的特殊规则。
...
::
    ...
    1 ]=> (load "p129-install-scheme-number-package.scm")                   ; 载入包
    ...
    1 ]=> (load "p129-generic-op.scm")                                      ; 载入通用操作
    ...
    1 ]=> (load "78-attach-tag.scm")                                        ; 载入重定义的三个函数
    ...
    1 ]=> (install-scheme-number-package)                                   ; 安装包
