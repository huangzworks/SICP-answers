...
- ``length-of-rectangle`` 接受一个矩形作为参数，并返回矩形的长（的长度）

- ``width-of-rectangle`` 接受一个矩形作为参数，并返回矩形的宽（的长度）
...
以下就是一个采用这种表示的矩形，其中矩形的长为一对长度为 4 的线段，而矩形的宽则是一对长度为 2 的线段：
...
以下就是一个采用这种表示的矩形，其中矩形的长为一条长度为 4 的线段，而矩形的宽则是一条长度为 2 的线段：
